...
function findFirstSingleChar(str) {
  return str.split('').find((char) => str.split(char).length - 1 === 1);
}
...
function isBracketsBalanced(str) {
  ...
  const stack = str.split('').reduce((acc, char) => {
    ...
    if (pairs[char]) {
      if (acc[acc.length - 1] !== pairs[char]) return [...acc, null];
      return acc.slice(0, -1);
    }
    return acc;
  }, []);

  return stack.length === 0 && !stack.includes(null);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const common = parts[0].reduce((acc, segment, i) => {
...
function getMatrixProduct(m1, m2) {
  return m1.map((row, i) =>
    m2[0].map((_, j) =>
      row.reduce((sum, __, k) => sum + m1[i][k] * m2[k][j], 0)
    )
  );
}
